...
namespace WerewolfKill.Utils.AzureStorage
{
    public class AzureTable
    {
        ...
        public async Task<IList<T>> FindAsync<T>(string partitionKey) where T : TableEntity, new()
        {
            ...
            {
                querySegment = await m_table
                    .ExecuteQuerySegmentedAsync(new TableQuery<T>()
                    .Where(TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, partitionKey)), querySegment != null ? querySegment.ContinuationToken : null);
